...
class IMessage;
namespace internal {
AGORA_API IMessage* AGORA_CALL createMessage(IRtmService* service,
                                             int message_type,
...
AGORA_API bool AGORA_CALL testFeatureEnabled(int peer_message_error_code);

// Whether enable same thread check, if enabled and some object is used in wrong
// thread, sdk will abort immediately
AGORA_API void AGORA_CALL enableSameThreadCheck(bool enable);
...
// Set client type,this API must be called before all rtm instance created and
// only once, otherwise function will return false,client type default is 39
